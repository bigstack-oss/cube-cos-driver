...
// LoadBalancer's own address: behind a terminator it is a hostname, through a
// tunnel it is localhost. A caller that says nothing gets https on the LB —
// the only default that can hold the Secure session cookie.
func TestAdvisorBaseURLDefaultsToTheLoadBalancer(t *testing.T) {
	if got := advisorBaseURL(InstallParams{AdvisorLBIP: "10.32.1.102"}); got != "https://10.32.1.102" {
